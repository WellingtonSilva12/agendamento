...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        const existingUser = await dbGet("SELECT id FROM users WHERE username = ? OR email = ? OR (matricula = ? AND matricula IS NOT NULL AND matricula != '')", [username, email, matricula]);
        if (existingUser) {
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        const payload = { 
            ...
            matricula: user.matricula, role: user.role 
        };
...
app.get('/api/reservas', async (req, res) => {
  try {
    const reservas = await dbAll(`
        ...
        FROM reservas r LEFT JOIN reserva_notebooks rn ON r.id = rn.reserva_id LEFT JOIN notebooks n ON rn.notebook_id = n.id
        GROUP BY r.id ORDER BY r.data_inicio DESC
    `);
    const reservasFormatadas = reservas.map(r => ({ ...r, notebooks: r.notebooks ? JSON.parse(r.notebooks).filter(n => n.id !== null) : [] }));
...
app.delete('/api/reservas/:id', async (req, res) => {
  try {
    ...
    await logToHistory('RESERVA_CANCELADA', { ...reserva, notebooks: notebooks, canceledBy: req.user.username });
